...
yellow "2：Использовать предыдущую стабильную версию ядра 1.10.7 (поддерживает разделение трафика по географическому признаку и переключение приоритетов IP-адресов)"
...
sbymfl
echo
[[ "$sbnh" != "1.10" ]] && wfl4='временно не поддерживается' sfl6='временно не поддерживается' adfl4='временно не поддерживается' adfl6='временно не поддерживается'
...
lnsb(){
rm -rf /usr/bin/sb
curl -L -o /usr/bin/sb -# --retry 2 --insecure https://raw.githubusercontent.com/MyNicknme/SB_new/refs/heads/main/SB.sh
chmod +x /usr/bin/sb
}
...
blue "Добро пожаловать снова в скрипт Sing-box-yg：bash <(curl -Ls https://raw.githubusercontent.com/MyNicknme/SB_new/refs/heads/main/SB.sh)"
echo
}
