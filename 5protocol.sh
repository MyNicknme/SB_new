...
yellow "1：Автоматически сгенерировать случайный порт для каждого протокола (в диапазоне 10000-65535), Enter по умолчанию"
yellow "2：Пользовательские порты для каждого протокола"
